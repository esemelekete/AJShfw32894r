							...
							<div class="col-md-8 col-lg-5">
								<div class="form-group">
									...
									<select name="id_ts" id="id_ts" class="form-control {if form_error('id_ts')}is-invalid{/if}">
										...
										{foreach $ts as $t}
											<option value="{$t->id}" {if set_value('id_ts') === $t->id}selected{/if}>{$t->ta_1}/{$t->ta_2}</option>
										{/foreach}
									</select>
									{if form_error('id_ts')}
									...
									{if $this->ion_auth->is_admin()}
										<select name="id_kriteria" id="id_kriteria" class="form-control {if form_error('id_kriteria')}is-invalid{/if}">
											...
											{foreach $kriteria as $k}
												<option value="{$k->id}" {if set_value('id_kriteria') === $k->id}selected{/if}>{$k->id}. {$k->nama}</option>
											{/foreach}
										</select>
								...
								<div class="form-group">
									...
									<textarea name="deskripsi" id="deskripsi" cols="30" rows="4" class="form-control">{set_value('deskripsi')}</textarea>
								</div>
